...
std::string ApgBuilder::calculateMD5(const std::string &filepath) const {
  if (filepath.empty()) {
  ...
  while (file.good()) {
    ...
    if (bytes_read > 0) {
      if (EVP_DigestUpdate(md5Context, buffer.data(),
...
void ApgBuilder::createMD5Sums(const std::string &directory,
                               const std::string &output) const {
  validateDirectoryPath(directory);
  validateOutputPath(output);

  std::cout << Color::CYAN
            << "Creating MD5 checksums for directory: " << directory
  ...
  for (const auto &entry : fs::recursive_directory_iterator(directory, ec)) {
    ...
    if (entry.is_regular_file(ec) && !ec) {
      try {
        ...
        std::cout << Color::GREEN << "✓ " << Color::RESET << relative_path
                  << std::endl;
        ++files_processed;
...
void ApgBuilder::validateDirectoryPath(const std::string &path) const {
  if (path.empty() || path.find("\0") != std::string::npos)
    throw std::invalid_argument("Invalid directory path");
}

void ApgBuilder::validateOutputPath(const std::string &path) const {
  if (path.empty() || path.find("\0") != std::string::npos)
    throw std::invalid_argument("Invalid output path");
}

void ApgBuilder::createArchive(const std::string &archive_path,
  ...
  try {
    ...
    for (const auto &dir_entry :
         fs::recursive_directory_iterator(source_dir, ec)) {
      if (ec) {
        throw std::runtime_error("Error iterating directory '" + source_dir +
                                 "': " + ec.message());
      }

      const std::string full_path = dir_entry.path().string();
      const std::string relative_path =
          fs::relative(dir_entry.path(), source_dir).string();
      if (stat(full_path.c_str(), &st) != 0) {
        std::cerr << Color::RED
                  << "Warning: Cannot stat file, skipping: " << full_path
                  << Color::RESET << std::endl;
        continue;
      }
      ...
      if (archive_write_header(a, entry) != ARCHIVE_OK) {
        std::cerr << Color::RED
                  << "Warning: Failed to write header for: " << relative_path
                  << " - " << archive_error_string(a) << Color::RESET
                  << std::endl;
      ...
      if (S_ISREG(st.st_mode)) {
        ...
        if (!fd) {
          std::cerr << Color::RED
                    << "Warning: Cannot open file, skipping: " << full_path
                    << Color::RESET << std::endl;
          archive_entry_free(entry);
          entry = nullptr;
          continue;
        ...
        while ((bytes_read = fread(buffer.data(), 1, BUFFER_SIZE, fd)) > 0) {
          if (archive_write_data(a, buffer.data(), bytes_read) < 0) {
            ...
            throw std::runtime_error(
                "Failed to write data for '" + relative_path +
                "': " + std::string(archive_error_string(a)));
          }
        }
    ...
    std::cout << Color::GREEN << "✓ Archive created with " << files_added
              << " files" << Color::RESET << std::endl;
  } catch (...) {
    if (fd)
      fclose(fd);
    if (entry)
...
bool ApgBuilder::isPathSafe(const std::string &path,
...
void ApgBuilder::extractArchive(const std::string &archive_path) const {
  struct archive *a;
  struct archive *ext;
  ...
  while (true) {
    ...
    if (!pathname) {
      std::cerr << Color::RED << "Warning: Entry with null pathname skipped"
                << Color::RESET << std::endl;
      continue;
    }
    ...
    if (!isPathSafe(pathname, extract_dir)) {
      std::cerr << Color::RED
                << "Warning: Unsafe path detected and skipped: " << pathname
                << Color::RESET << std::endl;
      continue;
    }
    ...
    if (file_size > static_cast<la_int64_t>(MAX_FILE_SIZE)) {
      std::cerr << Color::RED
                << "Warning: File too large, skipped: " << pathname
                << Color::RESET << std::endl;
      continue;
    }
    ...
    if (r != ARCHIVE_OK) {
      std::cerr << Color::RED
                << "Warning: Failed to write header for: " << pathname
                << Color::RESET << std::endl;
      continue;
    }
    ...
    if (archive_entry_size(entry) > 0) {
      ...
      while (true) {
        ...
        if (r != ARCHIVE_OK) {
          std::cerr << Color::RED
                    << "Warning: Data read error for: " << pathname
                    << Color::RESET << std::endl;
          break;
        }
        ...
        if (archive_write_data_block(ext, buff, size, offset) != ARCHIVE_OK) {
          std::cerr << Color::RED
                    << "Warning: Data write error for: " << pathname
                    << Color::RESET << std::endl;
          break;
        }
      }
    ...
    if (archive_write_finish_entry(ext) != ARCHIVE_OK) {
      std::cerr << Color::RED
                << "Warning: Failed to finish entry for: " << pathname
                << Color::RESET << std::endl;
    }
  }
...
std::string ApgBuilder::getInputLine(const std::string &prompt) const {
  std::string input;
  std::cout << prompt;
...
int ApgBuilder::getIntegerInput(const std::string &prompt) const {
...
void ApgBuilder::createMetadata() const {
  json metadata;
  ...
  metadata["architecture"] =
      getInputLine("Architecture (x86_64, aarch64, etc.): ");
  ...
  std::cout << Color::GREEN << "\nCreating metadata.json..." << Color::RESET
            << std::endl;
  ...
  std::cout << Color::GREEN << "✓ metadata.json created successfully!"
            << Color::RESET << std::endl;
}

void ApgBuilder::createPackage(const std::string &directory,
                               const std::string &output_path) const {
  validateDirectoryPath(directory);
  validateOutputPath(output_path);
  std::cout << Color::CYAN << "Creating package from directory: " << directory
            << Color::RESET << std::endl;
  if (!fs::exists(directory)) {
    throw std::runtime_error("Directory does not exist: " + directory);
  }
  ...
  std::cout << Color::GREEN << "✓ Package created successfully: " << output_path
            << Color::RESET << std::endl;
}

void ApgBuilder::extractPackage(const std::string &package_path) const {
  if (!fs::exists(package_path))
    throw std::runtime_error("Package not found");
  extractArchive(package_path);
  std::cout << Color::GREEN << "✓ Extracted successfully" << Color::RESET
            << std::endl;
}

void ApgBuilder::showVersion() const {
  std::cout << Color::BOLD << Color::CYAN << ApgBuilderInfo::DESCRIPTION
            << Color::RESET << std::endl
            << std::endl;
...
ApgBuilder::ProgramOptions ApgBuilder::parseArguments(int argc,
                                                      char **argv) const {
...
void ApgBuilder::showHelp() const {
  std::cout << Color::BOLD << Color::CYAN << ApgBuilderInfo::DESCRIPTION
            << Color::RESET << "\n"
            << "Usage: apgbuild [options] [directory]\n\n"
            << "Options:\n"
            << "  -m, --create-meta   Create metadata.json\n"
            << "  --open              Extract package\n"
            << "  --gen-sums          Generate MD5 sums\n"
            << "  -v, --version       Show version\n"
            << "  -o <path>           Output path\n";
}

void ApgBuilder::run(int argc, char **argv) {
  try {
